...
async function start() {
  ...
  const authServiceObj = await bus.getProxyObject('re.miru.AuthenticationService15', '/re/miru/AuthenticationService');
  const authService = authServiceObj.getInterface('re.miru.AuthenticationService15');
  ...
  const hostSessionObj = await bus.getProxyObject('re.miru.HostSession15', '/re/miru/HostSession');
  const hostSession = hostSessionObj.getInterface('re.miru.HostSession15');
  ...
  let agentSessionObj = await bus.getProxyObject('re.miru.AgentSession15', '/re/miru/AgentSession/' + sessionId[0]);
  let agentSession = agentSessionObj.getInterface('re.miru.AgentSession15');

  const sink = new MessageSink('re.miru.AgentMessageSink15');
  bus.export('/re/miru/AgentMessageSink/' + sessionId[0], sink);
...
setInterval(() => {
  ...
  peerChannel.onopen = async event => {
    ...
    const peerAgentSessionObj = await peerBus.getProxyObject('re.miru.AgentSession15', '/re/miru/AgentSession');
    const peerAgentSession = peerAgentSessionObj.getInterface('re.miru.AgentSession15');

    peerBus.export('/re/miru/AgentMessageSink', sink);
...
export default function createMiruBusPlugin(): Plugin<any> {
  return (store: any) => {
    start().catch(e => {
      console.error(e);
    });
