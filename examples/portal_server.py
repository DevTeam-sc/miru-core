...
from miru_tools.application import Reactor
...
class Application:
    def __init__(self):
        ...
        cluster_params = miru.EndpointParameters(
        ...
        if ENABLE_CONTROL_INTERFACE:
            ...
            control_params = miru.EndpointParameters(
                address="::1", port=27042, authentication=("callback", self._authenticate), asset_root=www
            )
        else:
            control_params = None

        service = miru.PortalService(cluster_params, control_params)
